...
class Datagroups2016(Datagroups):
    def __init__(self, infile, combine=False, pseudodata_pdfset = None, applySelection=True,
        ...
        self.groups =  {
            "Data" : dict(
                members = self.getSafeListFromDataset(["data"]),
                color = "black",
                label = "Data",
                selectOp = sigOp,
            ),
            "Zmumu" : dict(
                members = self.getSafeListFromDataset(["Zmumu"]),
                label = r"Z$\to\mu\mu$",
                color = "lightblue",
                selectOp = sigOp,
            ),   
            "Ztautau" : dict(
                members = self.getSafeListFromDataset(["Ztautau"]),
                label = r"Z$\to\tau\tau$",
                color = "darkblue",
                selectOp = sigOp,
            ),            
        ...
        if self.wmass:
            if splitWByCharge:
                self.groups.update({
                    "Wmunu_q1" : dict(
                        members = self.getSafeListFromDataset(["Wplusmunu"]),
                        label = r"W$^{-}\to\mu\nu$",
                        color = "darkred",
                        selectOp = sigOp,
                    ),
                ...
                self.groups.update({
                    "Wmunu_q0" : dict(
                        members = self.getSafeListFromDataset(["Wminusmunu"]),
                        label = r"W$^{-}\to\mu\nu$",
                        color = "darkred",
                        selectOp = sigOp,
                    ),
                ...
                self.groups.update({
                    "Wmunu" : dict(
                        members = self.getSafeListFromDataset(["Wminusmunu", "Wplusmunu"]),
                        label = r"W$^{\pm}\to\mu\nu$",
                        color = "darkred",
                        selectOp = sigOp,
                    ),
            ...
            self.groups.update({
                "Wtaunu" : dict(
                    members = self.getSafeListFromDataset(["Wminustaunu", "Wplustaunu"]),
                    label = r"W$^{\pm}\to\tau\nu$",
                    color = "orange",
                    selectOp = sigOp,
                ),
                "Top" : dict(
                    members = self.getSafeListFromDataset(["TTLeptonic", "TTSemileptonic", 
                        "SingleTschanLepDecays", "SingleTtWAntitop", "SingleTtchanAntitop", "SingleTtchanTop"
                        ]),
                    label = "Top",
                    color = "green",
                    selectOp = sigOp,
                ...
                "Diboson" : dict(
                    members = self.getSafeListFromDataset(["WW", "WZ", "ZZ2l2nu"]),
                    label = "Diboson",
                    color = "pink",
                    selectOp = sigOp,
                ), 
                "QCD" : dict(
                    members = self.getSafeListFromDataset(["QCDmuEnrichPt15"]),
                    label = "QCD MC",
                    color = "grey",
                    selectOp = sigOp,
                ), 
            ...
            self.groups["Other"] = dict(
                members = [x for x in self.datasets.values() if not x.is_data and x.name not in ["Zmumu", "Ztautau"] and x.name != "QCD"],
                label = "Other",
                color = "grey",
            )

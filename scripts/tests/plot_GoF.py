...
parser.add_argument(
    ...
    default=[],
    nargs="*",
    help="fitresult root file with test data (data, pseudodata)",
)
parser.add_argument(
    "-l",
    "--testLabels",
    default=[],
    nargs="*",
    help="labels for test data (data, pseudodata) for plotting",
)
parser.add_argument(
    "-n",
    "--nDoF",
    default=20,
    type=int,
    help="Number of degrees of freedom, for the chi2 function in the plot",
)
...
ax.step(
    bins, np.append(nEntries, nEntries[-1]), color="black", where="post", linewidth=0.5
)
...
for i, dt, l, p in zip(range(len(data)), data, testlabels, pvalues):
    ax.plot(
        [dt, dt], rangey, color=cm(i / len(data)), label=rf"{l} $p={round(p*100,1)}\%$"
    )
